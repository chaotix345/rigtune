...
def catalog(old_jar, new_jar, game_version, rules_files, now=None):
    ...
    paths. File URLs use the real CDN host, so downloads are checked against https://cdn.modrinth.com/."""
    ...
    for version_id, jar, published in (("E2Eold01", old_jar, now - datetime.timedelta(days=1)), ("E2Enew01", new_jar, now)):
        versions.append({
            ...
            "loaders": ["fabric"],
            "file": str(Path(jar).resolve()).replace("\\", "/"),
    ...
    return {
        ...
        "static": [{"host": RAW_HOST, "path": RULES_URL_DIR + Path(f).name, "file": str(Path(f).resolve()).replace("\\", "/"),
