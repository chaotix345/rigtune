...
// Audit M2 (docs/v0.4/SPEC.md 2o): staged groups the helper left half done at the last exit (its rollback failed, or it
// was killed): a DISABLE already done (its jar is gone and a .disabled copy is there) while an ENABLE of the same group
// isn't (its download is still there and its target isn't). Dropping such a group (Undo, Discard pending) would leave the
// disabled jar disabled and its replacement never enabled; the next exit finishes it instead.
