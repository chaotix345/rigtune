...
// MIN_GAP_PERCENT (twice BenchmarkMath.NOISY_CV: the quick protocol has one sample per side). Nothing is changed: the
// player picks a lighter profile in the pack's own settings. Iris has no API for profiles, and they're per pack.
public final class ShaderAdvice {
	...
	public static OptionalInt costPercent(SessionResult.@Nullable Cost shaders, double targetFps) {
		...
		boolean gap = shaders.lowGainPercent() >= MIN_GAP_PERCENT - 1e-9;
		if (!below || !reaches || !gap) {
			return OptionalInt.empty();
		}
		return OptionalInt.of((int) Math.round(100 * (shaders.offLow() - shaders.baselineLow()) / shaders.offLow()));
	}
}
