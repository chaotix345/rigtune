...
import io.github.chaotix345.rigtune.v030.core.benchmark.SessionResult.Measured;
...
public final class BenchmarkRecords {
	...
	public static BenchmarkRecord of(SessionResult r, BenchmarkRequest request, String phase, String id, String createdAt,
			String rigtuneVersion, String mcVersion, BenchmarkRecord.@Nullable World world, BenchmarkRecord.@Nullable Context context) {
		...
		return new BenchmarkRecord(id, createdAt, rigtuneVersion, mcVersion, request.mode().name(), request.scene().name(), phase,
				pairId, (int) Math.round(r.targetFps()), r.targetMet(), knobs, result, costs, r.notMeasured(), world, r.deadlineHit(), context);
	}
