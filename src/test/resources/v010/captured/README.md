...
the update. Only a passing run writes fixtures; `manifest.json` records the jars, the run, its verdict and each file's
sha256.
...
written with, so tests can run on Linux CI (Java on Windows accepts `/` too). In the JSON files only string values
that start with the instance path changed; the files were parsed and written back in Gson's two-space layout, so
field order and values are otherwise as 0.1.0 wrote them. Substitute your test's folder for the
