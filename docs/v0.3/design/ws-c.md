...
## Evidence
- Unit tests: `./gradlew build` green on 26.2 and 26.3, 907 tests each after merging feat/v0.3.0 with WS-F (1 skipped: the FIFO test on Windows; it runs on the Linux CI).
- Final CI run https://github.com/chaotix345/rigtune/actions/runs/36179340491 (head 52eb3e2, feat/v0.3.0 with WS-0 and WS-F merged in): every job green; the launcher screenshots and log lines are unchanged next to WS-F's Report a problem button. (Run 36177595248 had one leg fail on a transient Stonecutter plugin download before any code ran; re-run green.)
