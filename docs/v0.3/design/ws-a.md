...
- `checkUpdate(update, installedProjects, batch)`: the update's own version, checked the same way, with its own project's installed version ignored (it's the one being replaced). Another project's installed version counts even when that project is also being updated in the batch: refusing is safe, and the update is offered again next launch once the other one is installed. An installed mod's declaration that also matches the version being replaced (a whole-project entry against the updated mod) is a conflict that already exists, so it doesn't refuse the update (review finding 2); the update's own declarations against installed projects are always refused (AC3.2 iii).
- DownloadPlanner: every UpdateMod runs before any AddMod (selection order kept within each kind), so tick order doesn't change the outcome (AC3.2 iv). A staged update adds its version to `batch.versions` and its project to `groupOfUpdate`. An addition resolves with `updatedProjects = groupOfUpdate.keySet()` and joins the group of every project in `updatesNeeded`, so the addition and the update apply together or not at all; a failed update download never commits, so the addition is judged against the installed version and refused (AC3.2 v). An update missing from `updateVersions` (the online data changed between the report and the Apply) is refused before its download ("its Modrinth data changed since the list was made; try again"), since its own incompatibilities can't be checked (review finding 8; the plan had it checked as a bare version).
...
- After a queued drop, an undo and a discard, `recountStaged()` keeps a recommendation staged only while one of its op ids is still in pending.json and none of its enables is gone (an undo's re-enable of the same mod replaces an update's enable and takes over its disable: review finding 6), and sets `carriedOverOps` to the pending ops no staged recommendation owns (ops from another session, or 0.1.0 ops without ids). An unreadable pending.json changes nothing (finding 5).
...
## Known gaps
- Changes staged by an earlier Apply (already in pending.json, not in this batch) aren't part of the view, because pending.json has no Modrinth version ids (adding them would be a format change):
  - an addition incompatible only with the installed old version of a mod whose update is already staged is refused until that update has applied (safe; offered again after the restart);
  - the other side is let through: a later update whose new version declares an already-staged addition incompatible, or a later addition declaring an already-staged update's version incompatible, isn't caught.
- Two updates in one batch that Modrinth marks incompatible with each other's new versions: the one planned later is refused, so which one goes in follows tick order among updates (AC3.2 iv covers only additions against updates).
