...
| ws-h-knowledge | feat/knowledge | C:/Dev/Worktrees/rigtune-knowledge | running (Wave B) |
...
| ws-c-bench | feat/benchmark-v2 | C:/Dev/Worktrees/rigtune-bench | MERGED (18cd128); 421 tests per version; BENCHMARK_WORLD kept (it only touches the rigtune-benchmark save); AC6.4 checked with DH 3.3.2 in a production run. Phase 5 TODO: leave the benchmark world in a real NON-harness client (rigtune.dev.autorun hook, if WS-C added it; otherwise a manual check); shader cost report and Iris restore never exercised (no shader pack in any run). |
