...
**Tasks:** each fix with a test that fails first (SPEC ACs 2a.1-2m.1, plus amendment 2n: UndoPlanner's current value for a staged key = the last pending staged op; WS-A owns UndoPlanner for this fix); 2d is the first functional fix — do it first.
