...
**Tasks:** AC8.1-AC8.4 and AC9.1-AC9.7 (AC8.5, AC9.8 are Phase 5).
...
**Tasks:** AC10.1-AC10.6 and AC13.1-AC13.2. Land warn-only first; calibrate (1 local run under the lock + 3 CI runs per leg); switch to failing before the RC. Re-measure after WS-S merges (monitor on/off numbers) — that last step may be done by a follow-up branch.
