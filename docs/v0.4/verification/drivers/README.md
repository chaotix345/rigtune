...
| …and it parses to the expected family | `parse(AMD, VULKAN, "1.4.349 AMD proprietary driver 26.8.1 (LLPC)")` → **family unknown** (expected adrenalin 26.8.1) | **FAIL → P5A-F1 (low)** (`../P5A-FINDINGS.md`) |
...
## Test-vector suggestion (not applied: no product-code changes)
```java
