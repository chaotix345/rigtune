...
| **S-M1** phase-timer target missing | the game starts and shows "phase timing unavailable" | see "S-M1" below | see below |
...
phase timers). A2 (monitor on, 2,719 FPS, 0 spikes) shows no visible cost at high frame rates either.
...
- **DH- thread prefix at runtime:** see "Distant Horizons" below.
- **Autosave and client spikes:** see "Autosave" below.
