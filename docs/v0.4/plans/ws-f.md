...
- [ ] F1 `core/footprint/StartupTimesStore` + `ModSetHash` (TDD, `StartupTimesTest`, `ModSetHashTest`): startup-times.json
...
- [ ] F2 `client/footprint/StartupTimes`: records `RuntimeMXBean.getUptime()` at the first TitleScreen init (once per
...
- [ ] F3 `client/FootprintStats`: wall + render-thread CPU of RigTunePreLaunch, onInitializeClient and the CLIENT_STARTED
...
- [ ] F4 `tools/footprint-budgets.json` + `core/footprint/FootprintBudgets` (TDD, `FootprintBudgetsTest`): mode warn|fail,
      one limit + SPEC ceiling per metric, null metrics skipped, violations listed; limits never above ceilings (test).
- [ ] F5 `FrameHookBudgetTest` (AC10.1): `FrameTimes.onFrame` 10 M calls: allocation over the cold-to-hot run (so an
      allocation escape analysis would remove later still shows), ns/call over a second hot run.
- [ ] F6 `FootprintGameTest` (AC10.4, AC13.2): waits for the title screen, the report and the window; reads FootprintStats;
...
- [ ] F7 build.gradle: `-Drigtune.footprint.budgets` for `test` and `runProductionClientGameTest` (+ test input); a loopback
...
- [ ] F8 v040-written set `src/test/resources/v040-written/ws-f/startup-times.json`, written by `StartupTimesFixtureTest`
      through the store (the test fails if the committed file differs; `-Drigtune.fixtures.write=true` rewrites it); run
      WS-H's compat030 harness against it.
- [ ] F9 Calibration (AC10.3): one local 26.2 run under the machine lock (first in the lock order), 3 CI runs per leg;
      budget = min(ceiling, 2 × max observed); docs/v0.4/verification/footprint/README.md; switch to `fail`.
- [ ] F10 Gate proofs (AC10.2): 50 ms sleep in onInitializeClient → a red leg; `new long[1]` in onFrame → red unit test;
      both reverted; run URLs recorded.
- [ ] F11 README "RigTune's own footprint" (the startup A/B + the guard's numbers and budgets); verification README maps every
