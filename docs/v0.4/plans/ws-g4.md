...
- No run of 2: the old drop passes, a baseline last (residual: needs ~25 interleaved entries with staged changes).
- Review fixes: a chain break inside the run is kept as one older change (H1); the newest entry is never folded;
  the baseline id is `baseline-<uuid>` and kept across re-folds; each folded change keeps its newest change's id.
No new field, formatVersion 1: 0.3.0 reads, lists, undoes and rewrites the baseline as an ordinary Apply.

## Tasks
- [x] T1 M6 tests first (fail on 8869abc): `JournalFoldTest`
...
- [x] T2 implement the fold in Journal.cap; JournalTest, UndoPlannerTest, V030CompatTest green.
- [x] T3 0.3.0 round trip: `JournalFoldV030Test` (pinned v030 Journal state OK, HistoryModel lists the baseline as an
...
- [x] T4 L1 tests first: `LegacyImportTest` (`fromV010`: the 0.1.0 fixtures true; resultPath, PATCH_TOML/PROPERTIES,
  ...
  history.json: the new ops stay under the Apply's entry; the Staging case lives in HistoryStartupTest).
- [x] T5 implement: `LegacyImport.fromV010` gate in `HistoryStartup.legacyEntry`; `Staging.stage` creates a missing
  journal before the merge (so the first entry reads pending.json as it was).
- [x] T6 build both versions, self-review (code-reviewer subagent), merge origin/feat/v0.4.0, push, CI, design doc
