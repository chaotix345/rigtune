...
- **Keys** are `rigtune.<area>.<thing>`: `screen`, `header` and `category` for the main screen, `rec` for the recommendations RigTune writes itself ("Install %s", "Version %s is available (you have %s)."), `undo` and `history` for the Undo and History screens, `benchmark`, `launcher` (the memory steps for each launcher), `download` (why a download was refused), `status` and `toast` for messages, `settings` for the settings screen. Only the values are translated, never the keys.
- **Arguments:** `%s` is replaced by a value (a mod name, a number, another piece of text) in order. If your language needs them in a different order, number them: `%2$s ... %1$s`. Use exactly the arguments the English uses, no more and no fewer. `%%` is a percent sign (`"1%% low"` shows as "1% low"); a lone `%` makes Minecraft show the text unformatted.
...
- the recommendation text that comes from the rules (most titles and reasons, and the advice): it's written in English in [`rules/`](rules/) and downloaded from GitHub, so it stays English on a translated screen;
- mod names, versions, file names, and setting names and values taken from the rules or from the mods themselves;
- error details from outside RigTune's own checks: a network or file error after "Download failed:", the reason RigTune's apply helper gives for a change that failed at exit (History screen), and a failed benchmark check's error;
